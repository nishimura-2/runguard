...
SYSTEM_INSTRUCTION = (
    ...
    "メモリ使用率が高い／OOM のログがあれば 'out_of_memory'（推奨は 'scale_memory'＝メモリ上限引上げ）。"
    "外部依存先の障害に見えるなら 'dependency_5xx'（ロールバックは無意味なので 'escalate'）。"
    "起動時クラッシュの繰り返しは 'crash_loop'（直近デプロイ起因なら 'rollback'、そうでなければ 'restart'＝再起動）。"
    "デプロイが無いのにリクエストが異常に多いなら 'traffic_spike'（推奨は 'scale_instances'＝max-instances 増）。"
