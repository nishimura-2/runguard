...
| **新機能のバグ** | **まず即時ロールバックで止血 → 🔧 AIコード修正(self-heal)を提案** | 止血後、Gemini がバグだけ直した新版を提案 → **人が承認** → 新機能入りでデプロイ |

**2. 自己修復（self-heal）= 「止血してから、新機能ごと直す」** — 新機能にバグ（例: `/api/price` の 0除算）が入って 5xx になったとき、
RunGuard はまず**即座にロールバックして出血を止める**（新機能は一時停止するが、サービスは健全＝**承認待ちの間も安全＝夜も眠れる**）。
そのうえで traceback を読み、**新機能は残したままバグ箇所だけを修正した新リビジョンを提案**、ダッシュボードにコード差分（赤/緑）を表示。
**人が差分を承認**すると**新機能入りの修正版をデプロイ**し、復旧を検証（直らなければ自動でロールバック退避）。単純なロールバックと違い、最終的に**新機能を失わない**。
